...
- [x] **Step 1.5 — Matching algorithms**
...
- [x] **Step 5.3 — Result list + scrolling**
...
- [x] **Step 6.2 — Key bindings**
  - **Goal:** Same default bindings as C (move selection, accept, cancel, paste, etc.).
  - **Deliverables:** `libtofi::input::KeyAction` enum + `classify_keypress` (pure, fully unit-tested); `handle_keypress` in `libtofi::wayland` refactored to dispatch via `classify_keypress`; `auto_accept_single` fixed to fire after every bound key except Close/Submit (C parity); 47 new `classify_keypress` unit tests in `libtofi/src/input/tests.rs`.
  - **Verification:** `cargo test --workspace` — 122 tests pass; `cargo clippy` clean; manual checklist: all bindings listed in [`doc/tofi.1.md`](../doc/tofi.1.md) §KEYS are covered.
  - **C reference:** [`src/input.c`](../src/input.c)

- [x] **Step 6.3 — Pointer / hide cursor**
  - **Goal:** Pointer motion, hide cursor option.
  - **Deliverables:** `WaylandState` gains `pointer: Option<wl_pointer::WlPointer>` + `hide_cursor: bool`; `Dispatch<wl_seat::WlSeat>` updated to also acquire/release `wl_pointer`; `Dispatch<wl_pointer::WlPointer, ()>` impl — `Enter` calls `set_cursor(serial, None, 0, 0)` when `hide_cursor`; all other pointer events blank (matches C); `state.hide_cursor` wired from `TofiConfig::hide_cursor` in `main.rs`.
  - **Verification:** `cargo test --workspace` — 252+122 tests pass; `cargo clippy` clean; manual: `--hide-cursor=true` hides pointer when launcher has focus.
  - **C reference:** [`src/main.c`](../src/main.c)

- [x] **Step 6.4 — Modes: stdin / run / drun**
  - **Goal:** Populate `commands` from stdin, `$PATH` scan, or desktop list; `argv[0]` basename detection like C (`strstr` for `-run` / `-drun`).
  - **Deliverables:** `LaunchMode` enum + `detect_mode()` in `tofi-rs` (gated `wayland`); `read_stdin(normalize) -> Vec<String>` (NFC-normalises when `!ascii_input`); `sort_by_history` / `sort_drun_by_history` stable sort helpers; `entry.results` populated from the correct source per mode; history sort applied when `use_history` + path available; `WaylandState::drun_entries: Vec<DesktopEntry>` (feature `drun`) stored for Step 6.5; all items gated so `--no-default-features` stays clean.
  - **Verification:** `cargo test --workspace` 252+122 pass; `cargo clippy` clean (both feature sets); manual: `echo -e "foo\nbar" | tofi` lists stdin items; `tofi-run` lists `$PATH` executables; `tofi-drun` lists desktop apps.
  - **C reference:** [`src/main.c`](../src/main.c) post-config initialization

- [x] **Step 6.5 — Submit and stdout**
  - **Goal:** `do_submit` parity from [`src/main.c`](../src/main.c): `drun_launch`, `print_index`, history append.
  - **Deliverables:** `do_submit(state, config, mode, all_commands) -> bool` in `tofi-rs` (gated `all(wayland, renderer)`): handles empty-results case (echo raw input or reject for drun/require_match); resolves `abs_idx = (first_result + selection).min(results.len()-1)`; drun mode finds app by name in `state.drun_entries`, calls `drun_launch` or `drun_print`; stdin/run mode: `print_index` emits 1-based position in `all_commands`, else prints result; history update via `history::load`/`add`/`save` when `use_history` enabled; `drun_print` (gated `all(wayland, drun)`) expands exec, prepends terminal for terminal apps; `drun_launch` (gated `all(wayland, drun)`) splits exec on whitespace and spawns via `std::process::Command`; `all_commands` snapshot declared `#[cfg(feature = "renderer")] let all_commands: Vec<String>;` before renderer block, assigned inside (`entry.results.clone()`); submit handler in event loop gated `#[cfg(feature = "renderer")]` with `#[cfg(not(feature = "renderer"))]` break fallback; `fmt`+`clippy -D warnings` clean (both feature sets); 252+122 tests pass.
...
- [x] **Step 7.1 — Data device manager**
  - **Goal:** Bind `wl_data_device_manager`, create data device, handle paste shortcut.
  - **Deliverables:** `libtofi::wayland::clipboard::ClipboardState` (offer tracking, `begin_paste`, `reset`, `finish_paste`); `wl_data_device_manager` bound in registry; `wl_data_device` created in `connect()`; `Dispatch<WlDataDeviceManager>`, `Dispatch<WlDataDevice>` (with `event_created_child!` for opcode 0), `Dispatch<WlDataOffer>` (MIME negotiation) impls; `Paste` key action in `handle_keypress` calls `begin_paste()`; `read_clipboard()` reads non-blocking from pipe and inserts UTF-8 chars into entry; main-loop poll extended to include clipboard fd when active; 122 tests pass; fmt + clippy clean (both feature sets).
...
| Goal                    | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Config format**       | **Target:** TOML-only config (`tofi.toml`); drop the legacy keyfile parser (`load`/`apply_key` in `tofi::config::load`) entirely. End state: `serde` + `toml` crate deserialization directly into `TofiConfig`; no string-keyed `apply_key` dispatch; CLI flags still override via direct field assignment. Requires a **migration guide** and a deprecation release that reads both formats with a clear cutover date before removing the old parser. Split themes vs. app config is optional.                                                                                                                                                                                                                                                                                                                                                               |
| **Settings resolution** | Extract the priority merge logic out of `Cli::into_config` (currently in `tofi/src/cli/mod.rs`) into a dedicated **`tofi/src/settings.rs`**. The only rule: **CLI option > config file value > built-in default** — no other precedence magic. `cli.rs` becomes parse-only (clap struct, no merging); `config/` becomes load-only (file → raw values, no defaults); `settings.rs` owns the three-layer merge and produces the final `TofiConfig`. This makes the resolution rule explicit, testable in isolation, and easy to extend for the TOML migration and config/theme split below.                                                                                                                                                                                                                                                                     |
| **Config/theme split**  | Split the flat `TofiConfig` struct into two separate structs — `AppConfig` (geometry, font, text layout, behaviour) and `ThemeConfig` (colors, decoration, per-element themes, cursor theme) — parsed from separate files (`tofi.toml` vs. e.g. `themes/Sweet.toml`). The `include =` mechanism is replaced by an explicit `theme =` key or a CLI `--theme` flag. **Test requirement:** all three fixture patterns must have dedicated test coverage — **real-world** (`examples/config` + theme), **maximalistic** (`examples/config-complete`, every key set), **minimalistic** (`examples/config-minimal`, empty or required-only — currently empty since all keys have defaults; `TofiConfig::default()` must round-trip). Tests live in `config/tests.rs`; fixture files stay in `examples/`. This split is a prerequisite for the TOML migration above. |
...
- **2026-04-13:** **Phase 6 Step 6.3** — `WaylandState` gains `pointer: Option<wl_pointer::WlPointer>` + `hide_cursor: bool`; `Dispatch<wl_seat::WlSeat>` extended to acquire/release `wl_pointer` alongside keyboard (C: `wl_seat_capabilities`); `Dispatch<wl_pointer::WlPointer, ()>` — `Enter` event calls `pointer.set_cursor(serial, None, 0, 0)` when `hide_cursor` (C: `wl_pointer_enter`), all other events blank; `wl_pointer` added to protocol imports; `state.hide_cursor = config.hide_cursor` wired in `main.rs`; `fmt`+`clippy -D warnings` clean; 252+122 tests pass; §6 checkbox.
- **2026-04-13:** **Phase 6 Step 6.2** — `libtofi::input::KeyAction` enum (14 variants: `InsertChar(char)`, `DeleteChar`, `DeleteWord`, `ClearInput`, `Paste`, `PrevCursorOrResult`, `NextCursorOrResult`, `PrevResult`, `NextResult`, `PrevPage`, `NextPage`, `ResetSelection`, `Close`, `Submit`, `Unknown`); `classify_keypress(ctrl, alt, shift, key, ch) -> KeyAction` pure dispatcher covering all bindings from `doc/tofi.1.md` §KEYS; `handle_keypress` in `libtofi::wayland` refactored to dispatch via `classify_keypress` using a match expression; `auto_accept_single` bug fixed — now fires after every bound key except `Close`/`Submit`/`Unknown` (matching C's if-else fallthrough); 47 new `classify_keypress` unit tests in `input/tests.rs`; `cargo test --workspace` 122 tests pass; `fmt`+`clippy -D warnings` clean; §6 checkbox.
- **2026-04-10:** **Phase 6 Step 6.1** — `libtofi::input` module: `input/mod.rs` (Linux evdev key constants + pure text-editing helpers: `add_char`, `delete_char`, `delete_word`, `clear_input`); `input/keyboard.rs` (`RepeatInfo` struct with `timeout()`/`arm_repeat`/`disarm_repeat`/`advance_repeat`; `KeyboardState` wrapping `xkb::Context`/`Keymap`/`State` with `load_keymap`, `update_modifiers`, `set_repeat_info`, `key_repeats`, `key_get_utf32`, `mod_ctrl/alt/shift`, `keycode_to_linux_key`; `keysym_to_linux_key` maps `Keysym` → Linux evdev via `sym.raw()` match); 16 unit tests in `input/tests.rs`; `xkbcommon = "0.9"` added as optional dep behind `wayland` feature; `WaylandState` gains `keyboard`/`keyboard_state`/`entry`/`redraw`/`submit`/`physical_keybindings`/`auto_accept_single` fields (entry before surface for correct drop order); `Dispatch<wl_keyboard>` handles `Keymap` (fd→`read_to_string`), `Key` (arm/disarm repeat, call `handle_keypress`), `Modifiers`, `RepeatInfo`; `handle_keypress` public free fn handles printable insert, Ctrl shortcuts (W/H/U/V/C), cursor movement, result navigation, ESC/submit; `tofi/src/main.rs`: non-blocking event loop using `prepare_read`+`nix::poll`+`dispatch_pending` with key-repeat timeout, `state.entry` keeps Entry alive, submit prints selection, redraw calls `entry.update()`+`flush()`+`surface::draw`; `nix = {version="0.29", features=["poll"]}` added to `tofi-rs`; `fmt`+`clippy -D warnings` clean, 192+122 tests pass; §6 checkbox.
- **2026-04-10:** **Phase 5 Step 5.3** — `last_num_results_drawn: usize` field added to `Entry`; five selection-navigation methods added to `Entry`: `reset_selection` (zeroes `selection`+`first_result`), `select_next` (advances selection, scrolls `first_result` forward by page when boundary hit), `select_prev` (decrements selection, scrolls `first_result` back), `select_next_page` (jumps `first_result` forward by `num_results_drawn`), `select_prev_page` (jumps `first_result` back by `last_num_results_drawn`, clamps to 0); all methods mirror `src/input.c` (`select_next_result`, `select_previous_result`, `select_next_page`, `select_previous_page`, `reset_selection`); 13 new unit tests in `entry/tests.rs`; `fmt`+`clippy -D warnings` clean; §6 checkbox.
