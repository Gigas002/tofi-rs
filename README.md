...
| `1.0.0+` | `rust` | Opinionated fork — `drun`, `run`, and `dmenu` modes. Single binary: `tofi --mode drun` / `tofi --mode run` / `tofi --mode dmenu` (no `tofi-drun`/`tofi-run` symlinks). TOML config/theme, trimmed CLI surface. |
